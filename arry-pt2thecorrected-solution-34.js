...
const longestWord = (str) => {
    str = str.split(' ');
    let result = '';
    // the frist itration for result is (this) after that cann't be changed except in the (develpmet) becouse all str[i].length is less than (this)
    // the initiazler string dont chanched his value except the if condition is gonna be true 
    //str[i].length = 4 (this) and this is frist itration
...
const sumOfCubes = (myArray) => {
    ...
    for (let i = 0; i < myArray.length; i++) {
        result += myArray[i] ** 3;
        //myArray[i] = frist itration = 1 than 5 than 9 without muliply

        //result = frist itration = 1 than 6 than 15 without muliply
    }
    return result;
}
